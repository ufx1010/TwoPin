...
with sync_playwright() as p:
    ...
    # Test canvas rendering performance for kaleido layer
    test_canvas_code = """
    () => {
        // Implement canvas drawKaleidoscope
        window.drawKaleidoscopeCanvas = function() {
            ...
            if (b.patternType === 'kaleido') {
                ...
                for(let i=0; i < b.density; i++) {
                    ...
                    for(let j=0; j < segments; j++) {
                        ...
                        if(s.type === 'arc') {
                            ctx.beginPath();
                            ctx.ellipse(0, 0, size, Math.max(0.1, size*s.curve), 0, Math.PI, 2 * Math.PI);
                            ctx.globalAlpha = 0.6;
                            ctx.lineWidth = 0.8;
                            ctx.stroke();
                        } else if(s.type === 'blob') {
                            ...
                            ctx.beginPath();
                            ctx.arc(0, 0, Math.max(0.1, size/3), 0, Math.PI * 2);
                            ctx.globalAlpha = 0.5;
                            ctx.fill();
                        } else {
                            ctx.beginPath();
        ...
        drawKaleidoscopeCanvas();
    }
    """
    page.evaluate(test_canvas_code)
    ...
    t1 = time.time()
    print(f"Canvas export time: {t1 - t0:.2f}s")
